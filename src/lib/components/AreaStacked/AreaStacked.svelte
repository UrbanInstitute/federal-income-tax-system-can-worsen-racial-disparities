...
<script>
  ...
  const { data, xGet, yScale, zGet, zScale, z } = getContext("LayerCake");
...
<g class="area-group" clip-path="url(#clipMask)">
  {#each $data as d}
    <path class="path-area" d={areaGen(d)} fill={$zGet(d)} stroke="#ffffff"/>
  {/each}
</g>
