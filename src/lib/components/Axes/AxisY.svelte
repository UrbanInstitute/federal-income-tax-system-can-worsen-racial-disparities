...
<script>
  ...
  const { padding, xRange, yScale, xDomain, xScale, width } = getContext("LayerCake");

  /** @type {Boolean} [gridlines=true] - Extend lines from the ticks into the chart space */
  export let gridlines = true;

  /** @type {string} [gridlineColor="#DEDDDD"] - Fill color of grid lines */
  export let gridlineColor = '#DEDDDD';

  /** @type {Boolean} [tickMarks=false] - Show a vertical mark for each tick. */
  export let tickMarks = false;

  /** @type {Boolean} [tickLabels=true] - Show a text label for each tick. */
...
<g class="axis y-axis" transform="translate({-$padding.left}, 0)">
  ...
  {/each}
  {#if axisLabel}
    <text class="axis-label" x="0" y="-12" dy="0" dx={labelDx} fill={tickLabelColor}
      >{axisLabel}</text
    >
  {/if}
</g>
